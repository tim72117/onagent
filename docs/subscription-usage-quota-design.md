...
> **文件性質更新（已實作）**：本文原本是一份**設計提案**，撰寫當時 onagent 完全沒有計費 / 訂閱 / 配額機制。**這套設計後來已經實作並上線**，程式碼在 `backend/internal/quota/`（`quota.go`、`plan.go`、`admin.go`、`quota_test.go`），並串接到 `backend/internal/console/console.go`（`GET /console/quota`，開發者自助查詢自己的方案與用量）、`backend/internal/adminconsole/adminconsole.go`（後台的方案檢視與調整）、以及前端 `apps/console/src/api.ts`、`apps/admin/src/api.ts`。`backend/internal/db/schema.sql` 也已經有 `subscriptions` 與 `usage_events` 兩張表。
>
> 以下內文保留原本的分析與推理過程（為什麼選事件流水帳而不是計數器、為什麼週期用訂閱錨點而不是排程重置、執行點該放在握手還是每則訊息……），這些論證在實作後依然成立，是設計決策的紀錄，不是過時內容。**每個小節的實際落地情形，用引用區塊標注在該節末尾**——大多數決策都是「照設計實作」，但第 6 節的 schema 有一處實質差異（`monthly_quota` 從「方案本身的額度欄位」改成了「個別使用者的手動覆寫」，額度改成在程式碼的 `plans` 表中定義），詳見該節。
>
> 撰寫原文時檢查過 `docs/` 目錄下是否有先前的定價研究文件（搜尋 pricing / gemini / subscription 等關鍵字），**沒有找到**任何既有文件，因此本文不假裝延續某份不存在的定價策略，文中出現的方案額度數字僅作為示意；實作後這個數字（`plan.go` 的 `MonthlyPrompts: 100`）仍標注為 placeholder，尚未套用正式定價策略。
...
> **實作情形：完全照設計落地。** `inference.Request.AppID` 確實是實際歸屬鍵值（`backend/internal/quota/quota.go` 的 `Check`/`Record` 都以 `appID` 為入口參數）；`protocol.ErrorPayload.Code` 這個原本閒置的欄位，也確實被拿來放配額錯誤碼——見 `backend/internal/protocol/message.go:117` 的 `CodeQuotaExceeded = "quota_exceeded"`。
...
> **實作情形：照設計實作，選了做法二。** `backend/internal/quota/quota.go` 檔案開頭註解直接寫「the whole design is deliberately counter-free」，`Record` 就是一句 `INSERT ... ON CONFLICT (app_id, event_id) DO NOTHING`（無計數器、無鎖競爭），`usageSince` 則是即時 `COUNT(*)`。沒有引入 Redis 或任何額外元件。
...
> **實作情形：完全照設計實作，選了方案 B（從不重置，週期邊界永遠是查詢時算出來的）。** `backend/internal/quota/quota.go` 的 `currentPeriodStart(startedAt, now time.Time) time.Time` 正是本節描述的演算法：以 `started_at` 的「day of month」為錨點，找出小於等於 `now` 的最近一次月週期起點；月底邊界情況（錨點是 31 號但當月沒有 31 號）由 `monthBoundary`/`daysInMonth` 處理，clamp 到當月最後一天，行為與本節引用的 Stripe「Jan 31 錨點 → Feb 28/29 → Mar 31」完全一致，`quota_test.go` 也有對應的測試案例（`31st anchor clamps to February`、`leap-year February 29 exists` 等）。沒有任何排程 job；`nextPeriodBoundary` 是額外新增的輔助函式（設計文件未提及），用來算出「本期何時結束」給 `GET /console/quota` 顯示，同樣重用 `monthBoundary` 的 clamp 邏輯。
...
> **實作情形：完全照設計實作，連錯誤碼的字串都與提案內文一致。** 握手被拒回 `http.StatusTooManyRequests`（`backend/internal/ws/handler.go:152`，訊息為 "monthly quota exceeded for this app's plan"）。連線中被拒則是 `protocol.CodeQuotaExceeded = "quota_exceeded"`（`backend/internal/protocol/message.go:117`），由 `ws.Session.handlePrompt` 呼叫 `s.sendErrorCode(env.RequestID, "monthly prompt quota exceeded for this app's plan", protocol.CodeQuotaExceeded)`（`backend/internal/ws/session.go:227-228`）送出，連線本身確實沒有被關閉——`session.go` 的註解明講「the connection is NOT closed on rejection... upgrade their plan and keep using this same session」，與本節的建議一致。
...
> **實作情形：完全照設計實作，位置一 + 位置二都做了，位置三（per-tool-call）確實沒做。** `backend/internal/ws/handler.go` 的 `ServeHTTP` 在驗證通過後呼叫 `h.Quota.Check(checkCtx, appID)`（handler.go:146）；`backend/internal/ws/session.go` 的 `handlePrompt` 在呼叫 `s.infer.Complete` 之前呼叫 `s.quota.Check(ctx, app.AppID)`（session.go:223），成功後才呼叫 `s.quota.Record`（session.go:250）——連寫入順序都與第 6 節「先呼叫、確認成功後才記錄用量」的規劃一致。兩處都採「DB 錯誤 fail-open（記 log 但放行）」，這點提案原文沒有明講但屬於合理的實作細節，兩處程式碼註解都各自說明了理由（一次性的資料庫問題不該擋住合法使用者）。唯一一個提案沒提到的實作細節：握手時的配額查詢刻意不用 `r.Context()`，改用一個獨立的 3 秒 timeout context，理由是 `r.Context()` 會在客戶端快速重連時被取消，造成誤報的 fail-open 警告（handler.go:139-144 的註解）。

---

## 6. 具體最小 Schema 提案

> **本節有實質差異，請見節末的「實作情形」——`monthly_quota` 這個欄位的意義在實作中被改變了，建議完整讀過這節的差異說明再參考下方的原始提案 SQL。**
...
設計決策：**訂閱方案掛在 `users`**（付費關係天然是「一個開發者付一份錢」，不是「一個 App 付一份錢」，這也符合一個使用者可以擁有多個 `apps` 的現況），**用量事件掛在 `app_id`**（因為 `inference.Request.AppID` 是實際呼叫路徑上已經有的鍵值，不需要額外查表就能寫入），查詢「這個使用者目前用了多少」時再透過 `apps.owner_id` 把同一使用者名下所有 app 的用量加總。**這一段（訂閱掛在 user、用量以 app_id 記錄、查詢時透過 owner_id 加總）與實作完全一致，也回答了「per-app 還是 per-owner」這個問題：從最早的提案開始就是設計成 per-owner（同一使用者名下所有 app 共用同一份月配額），不是 per-app，實作沒有偏離這點。**

以下是**原始提案**的 SQL，保留作為設計推理的紀錄（**`subscriptions.monthly_quota` 的定義已經與實作不同，見本節最後的「實作情形」**）：

```sql
-- [原始提案，未採用此欄位定義 — 見本節末「實作情形」]
...
### 實作情形：`usage_events` 與查詢/寫入 SQL 照設計實作；`subscriptions.monthly_quota` 的意義有實質變更

`usage_events` 表（含兩個索引）與上面兩句查詢/寫入 SQL，在 `backend/internal/db/schema.sql` 幾乎逐字照抄提案實作，`internal/quota/quota.go` 的 `usageSince`／`Record` 也是同樣的 SQL。**這部分沒有差異，照設計實作。**

**有 divergence 的是 `subscriptions.monthly_quota` 這一欄位的意義**，實際的 schema（`backend/internal/db/schema.sql:127-139`）是：
...
-- for databases created under the old definition.
ALTER TABLE subscriptions ALTER COLUMN monthly_quota DROP NOT NULL;
```

`schema.sql` 自己的註解已經說明了這個變更的歷史：**`monthly_quota` 最初確實就是照提案的樣子實作的**（`NOT NULL`，存的是這個方案的實際額度）——但後來被改成一個**選填的個別使用者覆寫值**（`NULL` 為預設，代表「沒有特別覆寫，套用方案的額度」）。真正的方案額度改成寫死在程式碼裡：`backend/internal/quota/plan.go` 定義了一個 `plans map[Tier]Plan`，每個 `Plan` 有一個 `MonthlyPrompts int`（目前只有 `TierFree`，`MonthlyPrompts: 100`，仍標記為 placeholder），`PlanFor(tier)` 在每次檢查配額時即時查這個 map。

也就是說，額度的權威來源**從「每個使用者的 row 上各自一份數字」變成「每個 tier 在程式碼裡共用一份數字，個別 row 只能選擇性覆寫」**。`plan.go` 的註解直接點出了改這個設計的理由：「Quota enforcement (quota.go) resolves a user's tier to a Plan here at check time and never reads a per-row copy of the number — so changing a plan's `MonthlyPrompts` below immediately applies to every user on that tier, with no migration and no backfill.」——換句話說，提案原本的理由是「把額度存在 row 上，這樣改一個使用者的配額不需要 join 方案表」，但實作後發現真正常見的操作其實是反過來的：**調整「這個方案整體給多少」**（例如定價策略確定後，把 Free 方案從 100 次調成別的數字），如果額度是分散存在每一個使用者的 row 上，這種調整需要一次性 UPDATE 所有 row（或跑一次 migration/backfill）；改成程式碼裡的單一 map 之後，這種調整變成改一行程式碼、部署一次就對所有人立即生效，`monthly_quota` 欄位則保留下來，只做「這個人特別加開多少」的個案覆寫（`ownerStandingRow.limit()` 的邏輯是「有覆寫用覆寫，沒有就用方案值」，`quota_test.go` 的 `TestOwnerStandingLimit` 也涵蓋了這幾種情況，包括覆寫值為 0 的邊界案例）。

實務影響：`backend/internal/adminconsole/adminconsole.go` 的 `setUserPlan`／`quota.SetTier` 目前**只能改一個使用者的 tier**（例如把某人從 free 改成未來的 pro），還沒有後台介面可以直接設定 `monthly_quota` 這個個別覆寫值——目前這個覆寫欄位存在 schema 與 `quota.go` 的讀取邏輯裡，但寫入路徑（例如客服手動加開配額）還沒有對應的 API/UI，需要直接操作資料庫。

至於本節開頭提到的「per-app vs. per-owner」：**這點提案與實作從頭到尾一致，都是 per-owner**——`usageSince`／`ownerStanding`／`StandingFor` 都是以 `apps.owner_id` 為準，把同一使用者名下所有 app 的用量加總成一份共用配額，不是每個 app 各自一份。
...
## 實作總結（本節為文件更新時新增，非原始提案內容）

四點建議中，1、2、4 完全照設計實作，可對照上方每一節末尾的「實作情形」。第 3 點（超額行為）的錯誤回報機制（HTTP 429 + `protocol.ErrorPayload{Code: "quota_exceeded"}`）也完全照設計實作；但「把 `monthly_quota` 設計成可以透過後台手動調整的欄位」這句話在實作後有了更精確的答案——**它不是「方案額度本身可調」，而是分成兩層**：方案本身的額度現在是程式碼裡的 `plan.go` 常數（改一行程式碼、重新部署即可套用到整個 tier），`subscriptions.monthly_quota` 則收斂成單一使用者的個案覆寫值。細節見第 6 節「實作情形」。目前後台（`adminconsole`）只做到讓客服調整使用者的 tier，還沒有 API 可以寫入 `monthly_quota` 這個覆寫值——這是與本節建議「客服可以手動放寬」相比，目前仍缺的一塊。

尚未實作、與本文件無矛盾的部分：軟性超額（第 4 節提到的「付費方案可以軟性超額」）、降級服務、Redis 熱路徑計數器——這些原文就標註為「第一版不需要」或「未來考慮」，目前確實還沒有對應程式碼，屬於「按計畫暫緩」而非遺漏。另外，`GET /console/quota`（開發者自助查詢，`backend/internal/console/console.go:263`）與後台的方案檢視/調整 API（`backend/internal/adminconsole/adminconsole.go` 的 `listPlans`／`listUsers`／`setUserPlan`）是原提案沒有著墨的「報表/管理介面」層，屬於實作時的合理擴充，不影響本文件的核心設計判斷。

**總結一句話：本文件的核心工程判斷——事件流水帳取代計數器、週期邊界用訂閱錨點動態計算取代排程重置、握手+每則訊息雙層檢查、借用 `protocol.ErrorPayload.Code`、per-owner 而非 per-app 的配額範圍——全部照設計落地，沒有一項因為實作發現行不通而推翻。唯一的實質變更是 `subscriptions.monthly_quota` 從「方案額度本身」收斂為「個別使用者的覆寫值」，方案額度改放到程式碼的 `plans` map，原因是讓「調整整個方案的額度」不需要動使用者資料表；這是一個實作深化，不是設計失誤。**
